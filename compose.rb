...
- Ramblings: blog posts
- Publications: portfolio entries

eoh
type = ''
while true
  type = getinput("Is this a (r)ambling or (p)ublication?", "t")
  unless (type == '' or /^[rp]$/.match(type)) then redo end
  if type=='' then type = 'r' end
  break
end
...
first topic in the sub-category is listed in the ramblings index. Also note
